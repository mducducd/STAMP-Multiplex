...
def _forward_kronos(
    ...
    return MultiplexFeatures(
        feats=patch_embeddings,
        marker_embeddings=marker_embeddings,
        token_embeddings=token_embeddings,
    )


def kronos() -> MultiplexExtractor[torch.nn.Module]:
    model, _precision, _embedding_dim = create_model_from_pretrained(
        checkpoint_path="hf_hub:MahmoodLab/kronos",
    )

    return MultiplexExtractor(
        model=model,
        identifier=ExtractorName.KRONOS,
        forward=_forward_kronos,
    )
